...
| commarkai.info (Namecheap) | Secondary domain. Purpose: brand protection, plus hosting `support@commarkai.info` which is the recovery email for the Calendly account above. | Original registration lapsed; re-registering 2026-08-29 | Re-registration in progress | Namecheap account login | Registered at Namecheap (same account the domain was originally founded under). Not the primary domain: `commarkai.com` stays the business's real home for website, email, and marketing. Keep `commarkai.info` DNS minimal: Namecheap BasicDNS plus email forwarding MX only, no website, no marketing subdomains. Confirm the name is still available before assuming this path; if someone else has taken it, fall back to a Calendly support ticket or a fresh Calendly account. |
